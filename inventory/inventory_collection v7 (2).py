# ✅ FINAL FIXED VERSION OF YOUR AWS INVENTORY SCRIPT
...
def resolve_param(service, function_name, param, region):
    try:
        ...
        if service == "s3" and param.lower() == "bucket":
            buckets = client.list_buckets().get("Buckets", [])
            return buckets[0]["Name"] if buckets else "example-bucket"

        if service == "waf":
            if param.lower() in ["ruleid", "webaclid"]:
                response = client.list_web_acls()
                if response.get("WebACLs"):
                    return response["WebACLs"][0].get("WebACLId", "example-id")

        if service == "shield" and param.lower() == "resourcearn":
            return f"arn:aws:ec2:{region}:{AWS_SESSION.account_id}:instance/example-id"

        if service == "organizations" and param.lower() == "filter":
            return {"Type": "SERVICE_CONTROL_POLICY"}

        if service == "dynamodb" and param.lower() == "tablename":
            tables = client.list_tables().get("TableNames", [])
            return tables[0] if tables else "example-table"

        if param.lower().endswith(("id", "name", "arn", "type", "resource")):
            return f"example-{param.lower()}"
...
def call_function(service, function_name, region):
    ...
    if not hasattr(client, function_name):
        ...
        return {"error": f"Function {function_name} not found on boto3 client"}
...
def process_region_service(region, service, functions, results, errors, lock):
    for fn in functions:
        ...
        with lock:
            ...
            service_folder_name = f"{service}_{service_counts[account_id][service]}" if service_counts[account_id][service] > 1 else service

            path = OUTPUT_DIR / account_id / region / service_folder_name / fn
            path.mkdir(parents=True, exist_ok=True)
            output_file = path / f"{fn}_output_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"

            try:
                with open(output_file, "w") as f:
                    json.dump(output if "error" not in output else entry, f, indent=2, default=str)
                (errors if "error" in output else results).append(entry)
            except Exception as e:
...
def main():
    ...
    if access_key and secret_key:
        if not AWS_SESSION.initialize_session(access_key, secret_key): return
    else:
        if not AWS_SESSION.initialize_session(): return
    ...
    for _, row in df.iterrows():
        service = str(row.iloc[0]).strip().lower()
        region = str(row.iloc[1]).strip().lower()
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(process_region_service, region, service, service_function_map[service], results, errors, lock)
                   for region in enabled_regions for service in service_region_map[region] if service in service_function_map]
        for future in as_completed(futures):
            pass

    print(f"\n📊 Summary:\n  - Successful calls: {len(results)}\n  - Errors: {len(errors)}")
    stats = defaultdict(lambda: {'success': 0, 'errors': 0})
    for item in results: stats[item['service']]['success'] += 1
    for item in errors: stats[item['service']]['errors'] += 1
    print("\n🔍 Service-wise statistics:")
    for service, stat in sorted(stats.items()):
        print(f"  - {service}: ✅ {stat['success']} success, ❌ {stat['errors']} errors")
