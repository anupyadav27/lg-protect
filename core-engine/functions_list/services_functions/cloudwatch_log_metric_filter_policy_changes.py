#!/usr/bin/env python3
"""
cis_4.0_aws - cloudwatch_log_metric_filter_policy_changes
...
def load_compliance_metadata(function_name: str) -> dict:
    ...
    return {
        ...
        'risk_level': 'HIGH',
        'recommendation': 'Create log metric filter and alarm for IAM policy changes'
    }

# Load compliance metadata for this specific function
COMPLIANCE_DATA = load_compliance_metadata('cloudwatch_log_metric_filter_policy_changes')

def cloudwatch_log_metric_filter_policy_changes_check(logs_client, region: str, profile: str, logger) -> List[Dict[str, Any]]:
    """
    Perform the actual compliance check for cloudwatch_log_metric_filter_policy_changes.
    ...
    try:
        ...
        if not log_groups:
            findings.append({
                ...
                'recommendation': COMPLIANCE_DATA.get('recommendation', 'Create log metric filter and alarm for IAM policy changes'),
                'details': {
        ...
        # Check for CloudTrail log groups and metric filters for IAM policy changes
        cloudtrail_log_groups = []
        policy_change_filters = []
            ...
            if 'cloudtrail' in log_group_name.lower():
                ...
                try:
                    ...
                    for metric_filter in metric_filters:
                        ...
                        # Check if filter pattern matches IAM policy changes
                        if _check_policy_changes_pattern(filter_pattern):
                            policy_change_filters.append({
        ...
        for filter_info in policy_change_filters:
        ...
        has_metric_filter = len(policy_change_filters) > 0
        has_alarm = len(alarms_configured) > 0
        
        if has_metric_filter and has_alarm:
            status = 'COMPLIANT'
            compliance_status = 'PASS'
            recommendation = 'Log metric filter and alarm for IAM policy changes are properly configured'
        else:
            status = 'NON_COMPLIANT'
            compliance_status = 'FAIL'
            recommendation = COMPLIANCE_DATA.get('recommendation', 'Create log metric filter and alarm for IAM policy changes')
        ...
        finding = {
            ...
            'resource_id': f'policy-changes-monitoring-{region}',
            'status': status,
            ...
            'details': {
                ...
                'policy_change_filters': policy_change_filters,
        ...
        logger.error(f"Error in cloudwatch_log_metric_filter_policy_changes check for {region}: {e}")
        findings.append({
            'region': region,
            'profile': profile,
            'resource_type': 'CloudWatch Monitoring',
...
def _check_policy_changes_pattern(filter_pattern: str) -> bool:
    """
    Check if the filter pattern matches IAM policy changes monitoring.
    
    Args:
        filter_pattern (str): The CloudWatch Logs filter pattern
        
    Returns:
        bool: True if pattern matches IAM policy changes, False otherwise
    """
    ...
    # IAM policy related event names
    policy_events = [
        'deleterolepolicy',
        'deleteuserpolicy',
        'deletegrouppolicy',
    ...
    has_policy_event = any(policy_event in pattern_lower for policy_event in policy_events)
    
    # Check for general policy patterns
    has_policy_pattern = 'policy' in pattern_lower and ('create' in pattern_lower or 'delete' in pattern_lower or 'attach' in pattern_lower or 'detach' in pattern_lower or 'put' in pattern_lower)
    
    return has_eventname and (has_policy_event or has_policy_pattern)

def cloudwatch_log_metric_filter_policy_changes(profile_name: str = None, region_name: str = None) -> Dict[str, Any]:
    """Main compliance check function."""
    engine = ComplianceEngine(COMPLIANCE_DATA)
    return engine.run_compliance_check(
        check_function=cloudwatch_log_metric_filter_policy_changes_check,
...
if __name__ == "__main__":
    ...
    results = cloudwatch_log_metric_filter_policy_changes(
