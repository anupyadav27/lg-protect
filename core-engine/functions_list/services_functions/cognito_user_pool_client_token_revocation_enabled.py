#!/usr/bin/env python3
"""
soc2_aws - cognito_user_pool_client_token_revocation_enabled
...
def load_compliance_metadata(function_name: str) -> dict:
        ...
        for entry in compliance_data:
            if entry.get('Function Name') == function_name:
                return {
                    ...
                    'recommendation': entry.get('Recommendation', 'Enable token revocation for Cognito User Pool clients')
                }
    ...
    return {
        ...
        'risk_level': 'MEDIUM',
        'recommendation': 'Enable token revocation for Cognito User Pool clients'
    }

# Load compliance metadata for this specific function
COMPLIANCE_DATA = load_compliance_metadata('cognito_user_pool_client_token_revocation_enabled')

def cognito_user_pool_client_token_revocation_enabled_check(cognito_client, region: str, profile: str, logger) -> List[Dict[str, Any]]:
    """
    Perform the actual compliance check for cognito_user_pool_client_token_revocation_enabled.
    ...
    try:
        ...
        # Check each user pool's clients for token revocation settings
        total_clients = 0
        for user_pool in all_user_pools:
            ...
            try:
                ...
                # Check each client for token revocation settings
                for client in clients:
                    ...
                    try:
                        ...
                        # Check token revocation settings
                        ...
                        if enable_token_revocation:
                            status = 'COMPLIANT'
                            compliance_status = 'PASS'
                            recommendation = 'Cognito User Pool client has token revocation properly enabled'
                        else:
                            status = 'NON_COMPLIANT'
                            compliance_status = 'FAIL'
                            recommendation = 'Enable token revocation for Cognito User Pool client'
                        ...
                        finding = {
                            ...
                            'risk_level': COMPLIANCE_DATA.get('risk_level', 'MEDIUM'),
                            'recommendation': 'Unable to check client token revocation configuration due to access error',
                            'error': str(e),
                            'details': {
                                'user_pool_id': user_pool_id,
                                'user_pool_name': user_pool_name,
        ...
        logger.error(f"Error in cognito_user_pool_client_token_revocation_enabled check for {region}: {e}")
        findings.append({
            'region': region,
            'profile': profile,
            'resource_type': 'Cognito',
            ...
            'compliance_status': 'ERROR',
            'risk_level': COMPLIANCE_DATA.get('risk_level', 'MEDIUM'),
            'recommendation': COMPLIANCE_DATA.get('recommendation', 'Enable token revocation for Cognito User Pool clients'),
            'error': str(e)
        })
        
    return findings

def cognito_user_pool_client_token_revocation_enabled(profile_name: str = None, region_name: str = None) -> Dict[str, Any]:
    """Main compliance check function."""
    engine = ComplianceEngine(COMPLIANCE_DATA)
    return engine.run_compliance_check(
        check_function=cognito_user_pool_client_token_revocation_enabled_check,
...
if __name__ == "__main__":
    ...
    results = cognito_user_pool_client_token_revocation_enabled(
