#!/usr/bin/env python3
"""
soc2_aws - cognito_user_pool_password_policy_minimum_length_14
...
def load_compliance_metadata(function_name: str) -> dict:
        ...
        for entry in compliance_data:
            if entry.get('Function Name') == function_name:
                return {
                    ...
                    'recommendation': entry.get('Recommendation', 'Configure password policy minimum length to 14 characters')
                }
    ...
    return {
        ...
        'risk_level': 'MEDIUM',
        'recommendation': 'Configure password policy minimum length to 14 characters'
    }

# Load compliance metadata for this specific function
COMPLIANCE_DATA = load_compliance_metadata('cognito_user_pool_password_policy_minimum_length_14')

def cognito_user_pool_password_policy_minimum_length_14_check(cognito_client, region: str, profile: str, logger) -> List[Dict[str, Any]]:
    """
    Perform the actual compliance check for cognito_user_pool_password_policy_minimum_length_14.
    ...
    try:
        ...
        # Check each user pool for password policy minimum length requirement
        for user_pool in all_user_pools:
            ...
            try:
                ...
                minimum_length = password_policy.get('MinimumLength', 8)  # Default is 8
                ...
                if minimum_length >= required_minimum_length:
                    status = 'COMPLIANT'
                    compliance_status = 'PASS'
                    recommendation = f'Cognito User Pool password policy has adequate minimum length ({minimum_length} characters)'
                else:
                    status = 'NON_COMPLIANT'
                    compliance_status = 'FAIL'
                    recommendation = f'Set password minimum length to {required_minimum_length} characters or more (current: {minimum_length})'
                ...
                finding = {
                    ...
                    'details': {
                        ...
                        'current_minimum_length': minimum_length,
                        'required_minimum_length': required_minimum_length,
                        'meets_requirement': minimum_length >= required_minimum_length,
                        'password_policy': {
        ...
        logger.error(f"Error in cognito_user_pool_password_policy_minimum_length_14 check for {region}: {e}")
        findings.append({
            'region': region,
            'profile': profile,
            'resource_type': 'Cognito',
            ...
            'compliance_status': 'ERROR',
            'risk_level': COMPLIANCE_DATA.get('risk_level', 'MEDIUM'),
            'recommendation': COMPLIANCE_DATA.get('recommendation', 'Configure password policy minimum length to 14 characters'),
            'error': str(e)
        })
        
    return findings

def cognito_user_pool_password_policy_minimum_length_14(profile_name: str = None, region_name: str = None) -> Dict[str, Any]:
    """Main compliance check function."""
    engine = ComplianceEngine(COMPLIANCE_DATA)
    return engine.run_compliance_check(
        check_function=cognito_user_pool_password_policy_minimum_length_14_check,
...
if __name__ == "__main__":
    ...
    results = cognito_user_pool_password_policy_minimum_length_14(
