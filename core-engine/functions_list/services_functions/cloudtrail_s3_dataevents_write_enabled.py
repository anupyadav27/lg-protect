#!/usr/bin/env python3
"""
cis_1.5_aws - cloudtrail_s3_dataevents_write_enabled
...
def load_compliance_metadata(function_name: str) -> dict:
    ...
    return {
        ...
        'id': '3.1',
        'name': 'CloudTrail S3 Write Data Events',
        ...
        'risk_level': 'MEDIUM',
        'recommendation': 'Enable CloudTrail data events for S3 write operations to monitor object-level activities'
    }

# Load compliance metadata for this specific function
COMPLIANCE_DATA = load_compliance_metadata('cloudtrail_s3_dataevents_write_enabled')

def cloudtrail_s3_dataevents_write_enabled_check(cloudtrail_client, region: str, profile: str, logger) -> List[Dict[str, Any]]:
    """
    Perform the actual compliance check for cloudtrail_s3_dataevents_write_enabled.
    ...
    try:
        ...
        if not trails:
            findings.append({
                ...
                'recommendation': COMPLIANCE_DATA.get('recommendation', 'Create CloudTrail trails and enable S3 data events'),
                'details': {
        ...
        for trail in trails:
            ...
            try:
                ...
                for selector in event_selectors:
                    include_management_events = selector.get('IncludeManagementEvents', False)
                ...
                for advanced_selector in advanced_event_selectors:
                    ...
                    for field_selector in field_selectors:
                        ...
                        if field == 'eventCategory' and 'Data' in equals:
                            ...
                            if field == 'readOnly' and 'false' in equals:
                                has_write_events = True
                            elif field == 'eventName':
                                write_operations = ['PutObject', 'DeleteObject', 'CopyObject', 'RestoreObject']
                                if any(op in equals for op in write_operations):
                                    has_write_events = True
                    
                    if has_s3_resource and (not field_selectors or has_write_events):
                        has_s3_write_events = True
                        s3_write_details.append({
                ...
                trail_details = {
                    ...
                    's3_write_configuration': s3_write_details,
        ...
        for trail in compliant_trails:
            findings.append({
                ...
                'recommendation': 'S3 write data events are properly configured',
                'details': trail
            })
        ...
        for trail in non_compliant_trails:
            findings.append({
                ...
                'compliance_status': 'FAIL',
                'risk_level': COMPLIANCE_DATA.get('risk_level', 'MEDIUM'),
                'recommendation': COMPLIANCE_DATA.get('recommendation', 'Enable S3 write data events for this CloudTrail trail'),
                'details': trail
            })
        ...
        findings.append({
            ...
            'resource_id': f'all-trails-{region}',
            'status': summary_status,
            'compliance_status': summary_compliance,
            'risk_level': COMPLIANCE_DATA.get('risk_level', 'MEDIUM'),
            'recommendation': f'Ensure all CloudTrail trails have S3 write data events enabled. {len(compliant_trails)} compliant, {len(non_compliant_trails)} non-compliant',
            'details': {
        ...
        logger.error(f"Error in cloudtrail_s3_dataevents_write_enabled check for {region}: {e}")
        findings.append({
            'region': region,
            'profile': profile,
            'resource_type': 'CloudTrail',
            'resource_id': f's3-write-events-{region}',
            'status': 'ERROR',
            'risk_level': COMPLIANCE_DATA.get('risk_level', 'MEDIUM'),
...
def cloudtrail_s3_dataevents_write_enabled(profile_name: str = None, region_name: str = None) -> Dict[str, Any]:
    """Main compliance check function."""
    engine = ComplianceEngine(COMPLIANCE_DATA)
    return engine.run_compliance_check(
        check_function=cloudtrail_s3_dataevents_write_enabled_check,
...
if __name__ == "__main__":
    ...
    results = cloudtrail_s3_dataevents_write_enabled(
