...
class AccessAnalyzerService:
    """AccessAnalyzer service that collects resource data"""
    ...
    def _load_resources(self):
        """Load all AccessAnalyzer resources from AWS"""
        ...
        try:
            ...
            if not any(a.region == region for a in self.analyzers):
                placeholder_analyzer = Analyzer(
                    arn=f"arn:aws:access-analyzer:{region}:unknown:analyzer/unknown",
    ...
    def get_active_analyzers(self) -> List[Analyzer]:
        """Get only active analyzers"""
        return [a for a in self.analyzers if a.is_active]
    
    def get_analyzers_with_findings(self) -> List[Analyzer]:
        """Get analyzers that have findings"""
        return [a for a in self.analyzers if a.findings]
    
    def get_analyzers_with_active_findings(self) -> List[Analyzer]:
        """Get analyzers that have active findings"""
        return [a for a in self.analyzers if a.active_findings_count > 0]
