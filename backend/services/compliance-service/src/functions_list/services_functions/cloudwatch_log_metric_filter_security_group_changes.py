#!/usr/bin/env python3
"""
cis_4.0_aws - cloudwatch_log_metric_filter_security_group_changes
...
def load_compliance_metadata(function_name: str) -> dict:
    ...
    return {
        ...
        'risk_level': 'HIGH',
        'recommendation': 'Create log metric filter and alarm for security group changes'
    }

# Load compliance metadata for this specific function
COMPLIANCE_DATA = load_compliance_metadata('cloudwatch_log_metric_filter_security_group_changes')

def cloudwatch_log_metric_filter_security_group_changes_check(logs_client, region: str, profile: str, logger) -> List[Dict[str, Any]]:
    """
    Perform the actual compliance check for cloudwatch_log_metric_filter_security_group_changes.
    ...
    try:
        ...
        if not log_groups:
            findings.append({
                ...
                'recommendation': COMPLIANCE_DATA.get('recommendation', 'Create log metric filter and alarm for security group changes'),
                'details': {
        ...
        # Check for CloudTrail log groups and metric filters for security group changes
            ...
            if 'cloudtrail' in log_group_name.lower():
                ...
                try:
                    ...
                    for metric_filter in metric_filters:
                        ...
                        # Check if filter pattern matches security group changes
                        if _check_security_group_pattern(filter_pattern):
                            sg_change_filters.append({
        ...
        for filter_info in sg_change_filters:
        ...
        has_metric_filter = len(sg_change_filters) > 0
        has_alarm = len(alarms_configured) > 0
        
        if has_metric_filter and has_alarm:
            status = 'COMPLIANT'
            compliance_status = 'PASS'
            recommendation = 'Log metric filter and alarm for security group changes are properly configured'
        else:
            status = 'NON_COMPLIANT'
            compliance_status = 'FAIL'
            recommendation = COMPLIANCE_DATA.get('recommendation', 'Create log metric filter and alarm for security group changes')
        ...
        finding = {
            ...
            'resource_id': f'sg-changes-monitoring-{region}',
            'status': status,
            ...
            'details': {
                ...
                'sg_change_filters': sg_change_filters,
        ...
        logger.error(f"Error in cloudwatch_log_metric_filter_security_group_changes check for {region}: {e}")
        findings.append({
            'region': region,
            'profile': profile,
            'resource_type': 'CloudWatch Monitoring',
...
def _check_security_group_pattern(filter_pattern: str) -> bool:
    """
    Check if the filter pattern matches security group changes monitoring.
    
    Args:
        filter_pattern (str): The CloudWatch Logs filter pattern
        
    Returns:
        bool: True if pattern matches security group changes, False otherwise
    """
    ...
    # Security group related event names
    sg_events = [
    ...
    has_sg_event = any(sg_event in pattern_lower for sg_event in sg_events)
    
    # Check for general security group patterns
    has_sg_pattern = 'securitygroup' in pattern_lower
    
    return has_eventname and (has_sg_event or has_sg_pattern)

def cloudwatch_log_metric_filter_security_group_changes(profile_name: str = None, region_name: str = None) -> Dict[str, Any]:
    """Main compliance check function."""
    engine = ComplianceEngine(COMPLIANCE_DATA)
    return engine.run_compliance_check(
        check_function=cloudwatch_log_metric_filter_security_group_changes_check,
...
if __name__ == "__main__":
    ...
    results = cloudwatch_log_metric_filter_security_group_changes(
