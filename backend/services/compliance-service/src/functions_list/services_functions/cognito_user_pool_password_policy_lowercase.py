#!/usr/bin/env python3
"""
soc2_aws - cognito_user_pool_password_policy_lowercase
...
def load_compliance_metadata(function_name: str) -> dict:
        ...
        for entry in compliance_data:
            if entry.get('Function Name') == function_name:
                return {
                    ...
                    'recommendation': entry.get('Recommendation', 'Configure password policy to require lowercase letters')
                }
    ...
    return {
        ...
        'risk_level': 'MEDIUM',
        'recommendation': 'Configure password policy to require lowercase letters'
    }

# Load compliance metadata for this specific function
COMPLIANCE_DATA = load_compliance_metadata('cognito_user_pool_password_policy_lowercase')

def cognito_user_pool_password_policy_lowercase_check(cognito_client, region: str, profile: str, logger) -> List[Dict[str, Any]]:
    """
    Perform the actual compliance check for cognito_user_pool_password_policy_lowercase.
    ...
    try:
        ...
        # Check each user pool for password policy lowercase requirement
        for user_pool in all_user_pools:
            ...
            try:
                ...
                # Check if lowercase requirement is enabled
                ...
                if require_lowercase:
                    status = 'COMPLIANT'
                    compliance_status = 'PASS'
                    recommendation = 'Cognito User Pool password policy properly requires lowercase letters'
                else:
                    status = 'NON_COMPLIANT'
                    compliance_status = 'FAIL'
                    recommendation = 'Enable lowercase letter requirement in password policy'
        ...
        logger.error(f"Error in cognito_user_pool_password_policy_lowercase check for {region}: {e}")
        findings.append({
            'region': region,
            'profile': profile,
            'resource_type': 'Cognito',
            ...
            'compliance_status': 'ERROR',
            'risk_level': COMPLIANCE_DATA.get('risk_level', 'MEDIUM'),
            'recommendation': COMPLIANCE_DATA.get('recommendation', 'Configure password policy to require lowercase letters'),
            'error': str(e)
        })
        
    return findings

def cognito_user_pool_password_policy_lowercase(profile_name: str = None, region_name: str = None) -> Dict[str, Any]:
    """Main compliance check function."""
    engine = ComplianceEngine(COMPLIANCE_DATA)
    return engine.run_compliance_check(
        check_function=cognito_user_pool_password_policy_lowercase_check,
...
if __name__ == "__main__":
    ...
    results = cognito_user_pool_password_policy_lowercase(
