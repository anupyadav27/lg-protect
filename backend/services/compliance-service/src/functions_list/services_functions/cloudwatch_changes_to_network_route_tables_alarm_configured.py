#!/usr/bin/env python3
"""
kisa_isms_p_2023_aws - cloudwatch_changes_to_network_route_tables_alarm_configured
...
def load_compliance_metadata(function_name: str) -> dict:
    ...
    return {
        ...
        'description': 'Monitor changes to network route tables through CloudWatch alarms',
        ...
        'risk_level': 'MEDIUM',
        'recommendation': 'Configure CloudWatch alarms to monitor route table changes'
    }

# Load compliance metadata for this specific function
COMPLIANCE_DATA = load_compliance_metadata('cloudwatch_changes_to_network_route_tables_alarm_configured')

def cloudwatch_changes_to_network_route_tables_alarm_configured_check(logs_client, region: str, profile: str, logger) -> List[Dict[str, Any]]:
    """
    Perform the actual compliance check for cloudwatch_changes_to_network_route_tables_alarm_configured.
    ...
    try:
        ...
        route_table_alarms_configured = False
        metric_filters_found = []
        alarms_found = []
        
        # Check for metric filters monitoring route table changes
        for log_group_name in cloudtrail_log_groups:
            try:
                ...
                for metric_filter in response.get('metricFilters', []):
                    ...
                    # Check if the metric filter monitors route table changes
                    route_table_events = [
                    ...
                    if any(event in filter_pattern for event in route_table_events):
                            ...
                            if metric_name and metric_namespace:
                                try:
                                    ...
                                    for alarm in alarm_response.get('MetricAlarms', []):
                                        ...
                                        route_table_alarms_configured = True
        ...
        # Create findings
        if route_table_alarms_configured and metric_filters_found:
            finding = {
                'region': region,
                'profile': profile,
                'resource_type': 'CloudWatch Monitoring',
                'resource_id': f'route-table-change-monitoring-{region}',
                'status': 'COMPLIANT',
                'compliance_status': 'PASS',
                'risk_level': COMPLIANCE_DATA.get('risk_level', 'MEDIUM'),
                'recommendation': 'Route table change monitoring is properly configured',
                'details': {
            ...
            finding = {
                ...
                'risk_level': COMPLIANCE_DATA.get('risk_level', 'MEDIUM'),
                'recommendation': COMPLIANCE_DATA.get('recommendation', 'Configure CloudWatch alarms to monitor route table changes'),
                'details': {
                    'cloudtrail_log_groups': cloudtrail_log_groups,
                    'metric_filters_count': len(metric_filters_found),
                    'metric_filters': metric_filters_found,
                    'alarms_count': len(alarms_found),
                    'alarms': alarms_found,
                    'missing_configuration': 'Route table change monitoring alarms not properly configured'
                }
            }
        ...
        logger.error(f"Error in cloudwatch_changes_to_network_route_tables_alarm_configured check for {region}: {e}")
        findings.append({
            'region': region,
            'profile': profile,
            'resource_type': 'CloudWatch Monitoring',
...
def cloudwatch_changes_to_network_route_tables_alarm_configured(profile_name: str = None, region_name: str = None) -> Dict[str, Any]:
    """Main compliance check function."""
    engine = ComplianceEngine(COMPLIANCE_DATA)
    return engine.run_compliance_check(
        check_function=cloudwatch_changes_to_network_route_tables_alarm_configured_check,
...
if __name__ == "__main__":
    ...
    results = cloudwatch_changes_to_network_route_tables_alarm_configured(
