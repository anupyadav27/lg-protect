...
class rbac_minimize_webhook_config_access(KubernetesCheckBase):
    def execute(self) -> List[CheckResult]:
        ...
        try:
            ...
            for crb in cluster_role_bindings.items:
                for subject in crb.subjects:
                    if subject.kind in ["User", "Group"]:
                        result = CheckResult(
                            check_id="rbac_minimize_webhook_config_access",
                            check_name="RBAC Minimize Webhook Config Access",
                            status=CheckStatus.PASS,
                            status_extended=f"User or group '{subject.name}' does not have access to create, update, or delete webhook configurations.",
                        ...
                        # Check if the cluster role allows webhook config access
                        if crb.role_ref.name in cluster_role_rules:
                            if is_rule_allowing_permissions(
                                ...
                                result.status_extended = f"User or group '{subject.name}' has access to create, update, or delete webhook configurations."
            ...
            result = CheckResult(
                ...
                status_extended=f"Error checking RBAC webhook config access: {str(e)}",
                resource_id="",
                resource_name="",
                resource_type="User",
                namespace="cluster-wide",
