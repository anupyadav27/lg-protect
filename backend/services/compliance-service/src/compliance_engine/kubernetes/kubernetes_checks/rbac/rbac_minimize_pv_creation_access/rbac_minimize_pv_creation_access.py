...
class rbac_minimize_pv_creation_access(KubernetesCheckBase):
    def execute(self) -> List[CheckResult]:
        ...
        try:
            ...
            for crb in cluster_role_bindings.items:
                for subject in crb.subjects:
                    if subject.kind in ["User", "Group"]:
                        result = CheckResult(
                            check_id="rbac_minimize_pv_creation_access",
                            check_name="RBAC Minimize PV Creation Access",
                            status=CheckStatus.PASS,
                            status_extended=f"User or group '{subject.name}' does not have access to create PersistentVolumes.",
                        ...
                        # Check if the cluster role allows PV creation access
                        if crb.role_ref.name in cluster_role_rules:
                            if is_rule_allowing_permissions(
                                ...
                                result.status_extended = f"User or group '{subject.name}' has access to create PersistentVolumes."
            ...
            result = CheckResult(
                ...
                status_extended=f"Error checking RBAC PV creation access: {str(e)}",
                resource_id="",
                resource_name="",
                resource_type="User",
                namespace="cluster-wide",
