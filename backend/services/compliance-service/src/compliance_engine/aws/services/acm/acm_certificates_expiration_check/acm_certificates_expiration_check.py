...
from utils.reports.reporting import (
...
class acm_certificates_expiration_check(BaseCheck):
    ...
    def execute(self) -> list[CheckReport]:
        ...
        try:
            # Create service instance to get certificates
            session = boto3.Session()
            service = ACMService(session)
            certificates = service.get_all_certificates()
            ...
            report = CheckReport(
                ...
                status_extended=f"Error checking ACM certificates: {str(e)}",
                resource={"type": "acm", "id": "error"},
