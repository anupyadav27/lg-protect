...
class InventoryErrorAnalyzer:
    ...
    def auto_discover_latest_scan(self):
        ...
        # Use the actual scan results directory (corrected path)
        inventory_dir = Path(__file__).parent.parent.parent.parent.parent / "data/inventory/service_enablement_results"
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="AWS Inventory Error Analyzer")
    parser.add_argument('--scan_dir', type=str, help='Path to scan output directory')
    parser.add_argument('--error_csv_path', type=str, help='Path to error CSV file')
    args = parser.parse_args()

    analyzer = InventoryErrorAnalyzer(
        error_csv_path=Path(args.error_csv_path) if args.error_csv_path else None,
        scan_output_dir=Path(args.scan_dir) if args.scan_dir else None
    )
